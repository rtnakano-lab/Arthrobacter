# R script for pan-genome analysis using KEGG orthorogy from shimasaki et al., 2021
...
# directory
annotation        <- "/Users/shimasakitomohisa/Desktop/GenomeAnalysis/Genomes/arthrobacter/annotations/"
...
for (g in design$Strain) {            
  
path <- paste(annotation , g, ".ko", sep="")

df   <- read.table(path, fill=TRUE, header=FALSE, sep="\t", col.names=c("peg", "ko", "a", "b"))
...
ko.tab      <- t(ko.tab[, -1])
idx         <- rownames(ko.tab)
ko.tab      <- apply(as.matrix.noquote(ko.tab),2,as.numeric)
rownames(ko.tab)  <- idx
func.tab  <- (ko.tab > 0) * 1
...
ggsave(plot1, file=paste(Fig, "PCoA_KEGG.pdf", sep=""), width=6, height=5, bg="transparent")

## genome size comparison
genome_size  <- data.frame(size=colSums(ko.tab))
...
size_lineage <- ggplot(genome_size, aes(x=sublineage, y=size, color=sublineage)) +
  ...
  ylab("Number of KEGG Orthology")
 
ggsave(size_lineage, file=paste(Fig, "KEGG_size_lineages.pdf", sep=""), width=6, height=5, bg="transparent")
...
ggsave(size_origin, file=paste(Fig, "KEGG_size_origin.pdf", sep=""), width=6, height=5, bg="transparent")
...
## wilcox
ori_wilcox <-pairwise.wilcox.test(genome_size$size, genome_size$origin, p.adj="bonferroni", exact=F)
sink(paste(results, "wilcox_KEGG_origin.txt", sep=""))
print(ori_wilcox)
sink()

lineage_wilcox <-pairwise.wilcox.test(genome_size$size, genome_size$sublineage, p.adj="bonferroni", exact=F)
